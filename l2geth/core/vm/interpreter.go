...
func (in *EVMInterpreter) Run(contract *Contract, input []byte, readOnly bool) (ret []byte, err error) {
	...
	if in.cfg.Debug {
		defer func() {
			if err != nil {
				if !logged {
					in.cfg.Tracer.CaptureState(pcCopy, op, gasCopy, cost, &ScopeContext{mem, stack, contract}, in.returnData, in.evm.depth, err)
				} else {
	...
	for atomic.LoadInt32(&in.evm.abort) == 0 {
		...
		if in.cfg.Debug {
			in.cfg.Tracer.CaptureState(pc, op, gasCopy, cost, &ScopeContext{mem, stack, contract}, in.returnData, in.evm.depth, err)
			logged = true
		}
